...
폴더 구조와 함께 반드시 정해야 할 것이 **"어떤 파일을 버전 관리에 넣을 것인가"**입니다.

원칙은 하나입니다.

> **사람이 손으로 쓴 것만 넣고, 기계가 만들어낸 것은 넣지 않는다.**

이 프로젝트는 처음에 이 원칙을 안 지켜서 **추적 파일이 201개**였습니다. 정리 후 **22개**가 됐습니다.

| 넣으면 안 되는 것 | 왜 |
|---|---|
| `.pio/` | 빌드 산출물. 재생성 가능하고 수십 MB |
| `.cache/` | clangd 인덱스. **파일 178개**, 기계마다 다르고 빌드할 때마다 바뀜 |
| `compile_commands.json` | `pio run`이 자동 생성. **1.8MB**, 게다가 `/Users/내이름/...` 같은 **절대 경로가 박혀 있어** 다른 사람 컴퓨터에서는 무의미 |
| `.vscode/c_cpp_properties.json` 등 | 개인 환경 설정 |
| `.DS_Store` | macOS가 만드는 잡파일 |

⚠️ **이런 파일이 섞이면 실질적인 피해가 있습니다.**
- `git diff`에 자동 생성 파일의 변경이 수백 줄씩 섞여 **진짜 코드 변경이 안 보입니다**
- 저장소가 쓸데없이 무거워집니다
- 절대 경로가 들어간 파일은 **다른 사람이 받으면 오히려 빌드를 깨뜨립니다**

💡 **이미 커밋해버렸다면** — `.gitignore`에 추가하는 것만으로는 부족합니다. 이미 추적 중인 파일은 계속 추적되기 때문입니다.

```bash
git rm -r --cached .cache compile_commands.json
```

`--cached`가 핵심입니다. **git의 추적 목록에서만 빼고 디스크의 파일은 그대로 둡니다.** 이걸 빼먹으면 실제 파일이 지워져서 다시 빌드해야 합니다.
...
### 만들어뒀지만 안 쓰던 것들 — 그리고 그 결말

프로토타입을 만들다 보면 **"일단 규격은 정해뒀는데 아직 안 쓰는"** 메시지가 생깁니다. 이 프로젝트에는 둘 있었습니다.

| 메시지 | 처음 상태 | 검토 후 |
|---|---|---|
| `ping` / `pong` | 서버는 응답할 준비가 됐는데 브라우저가 안 보냄 | ✅ **13단계에서 하트비트로 실제 사용** — 좀비 연결 감지라는 진짜 문제를 해결 |
| `requestState` | 서버는 처리하는데 브라우저가 안 보냄 | 🔸 **그대로 둠** — 재연결 시 `join`을 다시 보내면 `state`가 따라오므로 아직 필요 없음 |

🔍 **안 쓰는 코드는 지워야 하나?** 판단 기준은 이렇습니다.

- **곧 쓸 곳이 명확하다** → 남기되 **문서에 "미사용"이라고 적기** (`requestState`가 이 경우)
- **쓸 곳이 있는데 안 쓰고 있었다** → **쓰면 됩니다** (`ping`이 이 경우였습니다)
- **왜 만들었는지 아무도 모른다** → **지우세요.** git에 남아 있으니 필요하면 되살릴 수 있습니다

⚠️ **가장 나쁜 것은 "안 쓰는데 문서에도 없는 코드"입니다.** 읽는 사람이 "이게 어디서 호출되지?" 하고 한참 찾다가, 결국 못 찾고는 **"내가 뭔가 놓쳤나?" 하며 코드를 못 건드리게 됩니다.**

> 💡 이 프로젝트에서 실제로 벌어진 일: `ping`/`pong`을 "미사용"으로 문서에 적어뒀더니, 나중에 좀비 연결 문제를 발견했을 때 **"아, 이미 서버에 준비돼 있잖아"** 하고 프론트엔드 15줄만 추가해서 끝났습니다. **넉넉하게 정해둔 규격이 값을 한 사례입니다.**
...
⚠️ **그런데 나눠놓기만 하면 끝이 아닙니다 — 매번 골라야 합니다.**
이 프로젝트를 검토했더니, **두 개념을 나눠놓고도 정작 쓸 때 틀린 곳이 세 군데** 있었습니다.

| 위치 | 원래 코드 | 무슨 일이 벌어졌나 |
|---|---|---|
| `handleStartGame()` | `connected`인 사람만 초기화 | 끊긴 플레이어가 이전 판의 `isTagger`를 들고 있어 **술래가 2명** |
| `isPositionOccupied()` | `connected`인 사람만 셈 | 끊긴 플레이어 위에 **장애물이 생성** |
| `handleJoin()` 재참가 | 연결 정보만 삭제 | 이전 방에 `connected=true`인 **유령 플레이어**가 영원히 남음 |

세 곳 모두 **"여기서 필요한 건 `active`인가 `connected`인가"를 한 번 더 생각했다면** 안 생겼을 버그입니다.

> 💡 **판단 기준**: **"이 플레이어가 게임에 존재하는가"를 묻는 자리에는 `active`**, **"지금 이 사람이 조작할 수 있는가"를 묻는 자리에는 `connected`**를 쓰세요.
> - 위치 배정, 역할 초기화, 자리 점유 → **`active`** (존재하니까)
> - 이동 처리, 회전 처리, 술래 뽑기 → **`connected`** (조작 가능해야 하니까)
...
### 배울 점 ⑤: 정리(cleanup)는 "짝"이 맞아야 한다

같은 연결이 `join`을 다시 보낼 때(다른 채널로 갈아타기 등) 원래 코드는 이것만 했습니다.

```cpp
freeConnection(client->id());   // ❌ 연결 정보만 지움
```

그런데 `ConnectionInfo`를 지운다고 `Player`가 정리되는 게 아닙니다. 이전 방의 플레이어는 **`connected = true`인 채로 남습니다.**

```
이전 방:  Player{ active=true, connected=true, clientId=17 }   ← 조작할 사람이 없음
                              ↑
        cleanupRooms()는 !connected 인 사람만 지운다  →  영원히 안 지워짐
```

슬롯이 8개뿐인데 하나가 영구히 묶입니다. **자원 누수(leak)** 입니다.

→ 연결 정보를 지우기 **전에** 그 플레이어를 먼저 "끊긴 것"으로 처리하도록 고쳤습니다.

```cpp
ConnectionInfo *prev = findConnection(client->id());
if (prev != nullptr && !prev->isDisplay && prev->roomIndex >= 0 && prev->playerIndex >= 0) {
    Player &prevPlayer = rooms_[prev->roomIndex].players[prev->playerIndex];
    if (prevPlayer.active && prevPlayer.clientId == client->id()) {
        prevPlayer.connected = false;   // ★ 이제 30초 뒤 정리 대상이 된다
...
💡 **일반 원칙 — 무언가를 두 곳에 등록했다면, 해제도 두 곳에서 해야 합니다.** 이 프로젝트는 하나의 접속을 `connections_[]`(연결 정보)와 `rooms_[].players[]`(게임 상태) **두 군데**에 기록합니다. 등록은 `handleJoin`에서 둘 다 하면서 해제는 한쪽만 하고 있었던 것이 원인입니다.
>
> 이런 버그를 찾는 요령: **"등록하는 곳"과 "해제하는 곳"을 나란히 놓고 개수를 세어보세요.**
...
2.5단계 서로 자리를 맞바꾸려는 쌍을 찾아 막는다   ← 나중에 추가됨 (아래 설명)
...
### 2.5단계: 서로 자리 바꾸기 막기 — 예약만으로는 못 잡는 구멍

**이 단계는 처음 만들 때 없었고, 나중에 코드를 검토하다 발견해서 추가한 것입니다.** 왜 필요한지 보세요.

```
[틱 시작]  술래(5,5)→오른쪽 이동     도망자(6,5)→왼쪽 이동
[목적지]   술래→(6,5)               도망자→(5,5)
[적용 후]  술래(6,5)                도망자(5,5)      ← 서로 자리를 맞바꿈
[잡기판정] 좌표가 다르므로 → 안 잡힘 ❌
```

아래 3단계의 칸 예약은 **"같은 칸을 노리는 경우"** 만 막습니다. 서로가 **떠날** 칸을 노리는 경우는 아무도 예약하지 않으므로 그냥 통과됩니다. 그 결과 **술래가 정면으로 달려오는 도망자를 관통해서 지나갑니다.** 게임이 성립하지 않는 수준의 버그입니다.
...
"내 목적지가 상대의 현재 위치이고, 상대의 목적지가 내 현재 위치인가" — 이 한 줄이 자리 바꾸기의 정의입니다.

막는 방법도 규칙을 정해야 합니다.

| 상황 | 처리 | 이유 |
|---|---|---|
| 술래 ↔ 도망자 | **도망자만** 제자리에 세운다 | 술래가 그 칸으로 들어와 정상적으로 잡힘. "정면으로 뛰어들면 잡힌다"가 직관에도 맞음 |
| 도망자 ↔ 도망자 | **둘 다** 제자리 | 서로 부딪혀 튕긴 셈 |

💡 **왜 3단계보다 먼저 해야 하나?** 여기서 누군가를 제자리에 세우면 "그 칸은 계속 점유된다"는 사실이 바뀝니다. 3단계 예약이 그 정보를 알아야 하므로 **반드시 앞에 와야** 합니다. **단계를 추가할 때는 "어디에 끼워 넣어야 하는가"가 코드 자체보다 중요할 때가 많습니다.**
...
> 📌 **이 함수는 실제로 이 문서를 쓰면서 버그가 발견되어 고쳐졌습니다.** 위 2.5단계가 그 결과물입니다. 처음 설계한 6단계 순서는 옳았지만, **"같은 칸을 노리는 충돌"만 생각하고 "자리를 맞바꾸는 충돌"은 떠올리지 못했던 것**입니다.
>
> 💡 **교훈 — 격자 위의 이동에는 충돌이 두 종류 있습니다.**
> 1. **정면 충돌**: 둘이 같은 칸을 원함 → 예약으로 해결
> 2. **자리 바꾸기**: 둘이 서로의 칸을 원함 → 별도 검사 필요
>
> 격자 기반 게임(로그라이크, 퍼즐, 턴제 전략)을 만들 때 거의 항상 마주치는 문제이니 기억해두세요.

✍️ **연습**: 세 명이 원형으로 자리를 도는 경우(A→B의 칸, B→C의 칸, C→A의 칸)는 어떻게 될까요? 지금 코드는 **두 명씩 짝지어서만** 검사하므로 이 경우는 막지 못합니다. 다만 세 명이 서로를 통과하는 것이 아니라 원형으로 **한 칸씩 밀려 도는** 것이라 게임상 문제가 없어서 그대로 뒀습니다. 만약 막아야 한다면 어떻게 검사해야 할까요?
...
⚠️ **원래 프론트엔드에는 두 가지 문제가 있었고, 검토 후 고쳤습니다.**
...
**① 같은 이름이 두 번** 나와서 문장이 어색했습니다. 서버가 `previousTaggerPlayerId`(잡은 사람)를 보내주고 있는데도 안 쓰고 있었습니다.

**② `showError()`로 띄워서 붉은 오류 색으로 보였습니다.** 누가 잡히는 것은 **정상적인 게임 진행**이지 오류가 아닙니다. 사용자에게 "뭔가 잘못됐다"는 잘못된 신호를 줍니다.
...
💡 **`else` 절에 주목하세요.** 잡은 사람이 그 사이에 나가버려서 목록에 없을 수도 있습니다. **"찾아본 결과가 없을 수 있다"는 경우를 항상 처리**하세요. 없으면 `catcher.name`에서 `undefined` 오류가 나고, JS는 그 시점에 함수 실행을 통째로 중단해버립니다.

💡 **오류와 알림은 색을 구분하세요.** 같은 배너를 재사용하되 CSS 클래스만 바꾸면 됩니다(`showError` / `showNotice`). **"모든 알림을 오류로 띄우면 사용자는 곧 모든 알림을 무시하게 됩니다."**
...
### 배울 점 ④: 불변식(invariant)을 지키는 검사는 딱 한 곳이면 된다

이 함수를 검토하다 **버그 두 개와 죽은 코드 하나**를 찾았습니다. 셋 다 같은 뿌리에서 나왔습니다.

이 프로젝트에는 이런 **불변식**이 있습니다.

> **"플레이어는 절대 장애물 위에 있을 수 없다."**

불변식이란 **"프로그램이 도는 내내 항상 참이어야 하는 명제"**입니다. 이게 깨지면 그 뒤의 모든 코드가 잘못된 전제 위에서 동작합니다.

이 불변식이 깨질 수 있는 경로는 **딱 두 개**뿐입니다.

| 경로 | 막는 방법 |
|---|---|
| 플레이어가 장애물 칸으로 **이동** | 9단계 1~2단계의 `tile == Obstacle → continue` |
| 장애물이 플레이어 칸에 **생성** | 여기 3번의 `isPositionOccupied()` 검사 |

그런데 원래 코드에는 문제가 있었습니다.

**① 배치 검사가 반쪽짜리였다** (진짜 버그)
`isPositionOccupied()`가 `p.connected`인 사람만 셌습니다. 잠깐 끊긴 플레이어(`active && !connected`)의 칸은 "비었다"고 판단되어 그 위에 장애물이 놓이고, 재접속하면 **장애물 안에 갇힌 채로 시작**했습니다.
→ `connected` 조건을 빼서 `active` 전체를 세도록 수정했습니다.

**② 시작 위치의 뒷문이 열려 있었다** (진짜 버그)
```cpp
p.x = found ? sx : 1;   // ❌ 빈 칸을 못 찾으면 무조건 (1,1)
```
(1,1)이 장애물이면 그대로 불변식이 깨집니다.
→ 자리를 못 찾으면 **참가를 거절**하도록 수정했습니다.

**③ 제거 쪽 검사는 죽은 코드였다**
```cpp
// 플레이어가 인접해있으면 제거하지 않는다   ← 주석부터 사실과 다름
if (!isPositionOccupied(room, ox, oy)) { ...제거... }
```
장애물을 **없애는** 것은 길을 넓히는 일이라 아무에게도 해가 없습니다. 게다가 불변식에 따라 그 칸에 플레이어가 있을 수 없으므로 **이 조건은 항상 참**이었습니다. 주석은 "인접"이라고 했지만 코드는 "정확히 그 칸"을 봤고요.
→ **삭제**하고, 불변식을 실제로 지키는 것은 배치 쪽 검사라는 점을 주석으로 남겼습니다.

💡 **여기서 배울 것**
1. **불변식을 문장으로 적어보세요.** "X는 절대 Y일 수 없다"를 명확히 하면, 그걸 깨뜨릴 수 있는 경로를 빠짐없이 셀 수 있습니다.
2. **경로를 다 세었으면 각 경로에 검사를 하나씩** 두세요. 그 외의 검사는 대개 죽은 코드입니다.
3. **죽은 코드는 없는 코드보다 나쁩니다.** 읽는 사람이 "여기서 뭔가 막아주고 있구나"라고 믿게 만들어, 진짜 구멍(위 ①②)을 못 보게 합니다.

### 아직 남은 문제: 갇힘

플레이어는 앞으로만 이동하고 후진이 없으므로, 사방이 막히면 회전만 하다 영영 못 움직입니다. 이건 아직 안 고쳤습니다(부록 B-11).

✍️ **연습**: 장애물을 놓기 전에 "이 칸을 막아도 각 플레이어가 갈 수 있는 칸이 남는가"를 BFS(너비 우선 탐색)로 검사해보세요. 12×12 격자라 비용은 무시할 만합니다.
...
### common.js: 하트비트 — 자동 재연결만으로는 부족한 이유

위의 자동 재연결에는 **치명적인 사각지대**가 있습니다. `onclose`가 불려야만 동작한다는 것입니다.

```
정상적인 종료:  보드가 소켓을 닫음 → TCP FIN 도착 → onclose 발생 → 재연결 ✅

Wi-Fi 범위 이탈:  신호가 그냥 사라짐 → FIN이 안 옴 → onclose 안 불림 ❌
보드 전원 차단:   갑자기 꺼짐      → FIN이 안 옴 → onclose 안 불림 ❌
```

이때 브라우저는 **연결이 살아있다고 착각한 채 아무 메시지도 못 받습니다.** 화면은 그냥 멈추고, "연결됨"이라고 표시되어 있으며, 자동 재연결도 동작하지 않습니다. 이것을 **좀비 연결**이라고 합니다.

⚠️ **이게 실제 현장에서 가장 흔한 증상입니다.** "갑자기 화면이 멈췄어요"의 대부분이 이것이며, 사용자는 새로고침 말고는 할 수 있는 게 없습니다.

해결책은 **직접 주기적으로 확인하는 것**입니다. 5단계에서 규격만 정하고 안 쓰던 `ping`/`pong`이 여기 쓰입니다.

```js
var PING_INTERVAL_MS = 10000; // 10초마다 ping
var PONG_TIMEOUT_MS  = 25000; // 25초간 아무 것도 못 받으면 죽은 것으로 간주
...
pingTimer = setInterval(function () {
  ...
  if (Date.now() - lastMessageAt > PONG_TIMEOUT_MS) {
    stopPing();
    socket.close();   // ★ 직접 닫으면 onclose가 불려서 재연결 경로를 탄다
    return;
  }
...
socket.onmessage = function (event) {
  lastMessageAt = Date.now();   // ★ 어떤 메시지든 "살아있음"의 증거
  ...
  if (data.type === 'pong') return;   // 확인용이라 따로 할 일 없음
};
```

💡 **설계 요령 세 가지**

1. **`pong`만 세지 말고 "어떤 메시지든" 받으면 갱신하세요.** 게임 중에는 `state`가 0.4초마다 오므로 사실상 ping을 보낼 일도 없습니다. 대기 중일 때만 ping이 실제로 필요합니다.
2. **타임아웃은 ping 간격의 2배 이상**으로 잡으세요. 여기서는 10초 간격에 25초 타임아웃이라 **ping을 두 번 연속 놓쳐야** 끊습니다. 잠깐 느려진 것만으로 끊어지면 오히려 더 불안정해집니다.
3. **직접 `close()`를 부르는 것이 핵심입니다.** 별도의 재연결 로직을 새로 짜지 않고, **이미 있는 `onclose` → 재연결 경로를 재사용**합니다. 새 코드를 만드는 대신 기존 경로로 흘려보내면 버그가 생길 자리가 줄어듭니다.

> 📌 이렇게 해서 **"서버에는 있는데 아무도 안 쓰던 코드"가 실제로 쓰이게 됐습니다.** 5단계에서 규격을 넉넉히 정해둔 것이 여기서 값을 한 셈입니다.
...
|---|---|
| `GameServer::loop()` | 메인 |
| `handleMessage()` | AsyncTCP (WS_EVT_DATA) |
| `handleDisconnect()` | **메인** — 원래는 AsyncTCP였지만 데드락 때문에 옮겼습니다 (아래 참고) |
...
### 🔥 실제로 있던 데드락 — 락 순서 역전(lock-order inversion)

여기까지가 "내 락 하나만 잘 쓰면 된다"는 이야기였습니다. 그런데 **이 프로젝트에는 실제로 보드 전체가 멈추는 데드락이 있었습니다.** 원인은 우리 락이 아니라, **우리 락과 라이브러리 락의 순서**였습니다.

#### 발견 과정

`ESPAsyncWebServer`도 자기 내부에 락을 갖고 있습니다 (라이브러리 소스를 직접 열어서 확인했습니다).

| 라이브러리 락 | 보호 대상 |
|---|---|
| `_ws_clients_lock` | 접속 클라이언트 목록 |
| `_queue_lock` | 클라이언트별 전송 대기열 |

문제는 **이 락들을 잡는 순서가 두 태스크에서 서로 반대**였다는 것입니다.
...
[메인 태스크 : 게임 틱]
  loop()
   └ mutex_ 획득 ★
      └ broadcastState()
         └ ws_.client(id)
            └ _ws_clients_lock 획득 시도 ☆      ← ★ 잡고 ☆ 를 원함

[AsyncTCP 태스크 : 연결 종료]
  AsyncWebSocket::_handleDisconnect()
   └ _ws_clients_lock 획득 ☆
      └ _clients.erase()            ← 여기서 클라이언트 객체가 소멸
         └ ~AsyncWebSocketClient()
            └ WS_EVT_DISCONNECT 발생
               └ 우리 handleDisconnect()
                  └ mutex_ 획득 시도 ★         ← ☆ 잡고 ★ 를 원함
```

**두 태스크가 서로가 가진 것을 원합니다.** 게다가 `portMAX_DELAY`라 타임아웃도 없어서, 한번 걸리면 **영원히** 멈춥니다.

⚠️ **얼마나 위험한가**: 게임 틱은 0.4초마다 돌고, 연결 종료는 아무 때나 일어납니다. 창이 좁아서 100번 중 99번은 무사히 지나가지만, **몇 시간 플레이하면 언젠가는 맞물립니다.** 그리고 걸리는 순간 보드가 통째로 죽습니다. 시리얼 로그도 안 나오고, 원인을 추적할 단서가 거의 없습니다.

#### 핵심 통찰: 콜백은 "누가 부르는가"가 중요하다

이 버그의 근본 원인은 이것입니다.

> **라이브러리가 자기 락을 잡은 채로 내 콜백을 불렀다.**

라이브러리 소스를 열어보기 전까지는 알 수 없는 사실입니다. 실제로 확인해보니 이벤트마다 달랐습니다.

| 이벤트 | 라이브러리 락을 잡은 채 호출되나? | 우리 핸들러가 `mutex_`를 잡나? | 위험? |
|---|---|---|---|
| `WS_EVT_CONNECT` | ✅ 잡은 채 호출됨 | ❌ 로그만 찍음 | 안전 |
| `WS_EVT_DATA` | ❌ 락 없이 호출됨 | ✅ 잡음 | 안전 |
| **`WS_EVT_DISCONNECT`** | ✅ **잡은 채 호출됨** | ✅ **잡음** | 🔥 **데드락** |

**둘 다 ✅인 칸이 하나라도 있으면 데드락입니다.** 이 표를 그려보는 것이 검증 방법입니다.

#### 해결: 락을 잡는 순서를 하나로 통일한다

데드락을 없애는 방법은 **"모든 태스크가 항상 같은 순서로 락을 잡게 만드는 것"**입니다. 여기서는 `mutex_ → 라이브러리 락` 하나로 통일하기로 했습니다.

그러려면 `WS_EVT_DISCONNECT`에서 `mutex_`를 잡지 않아야 합니다. 그래서 **"할 일을 적어두고 나중에 처리"**하는 방식으로 바꿨습니다.

```cpp
// ❌ 고치기 전 — 라이브러리 락을 잡은 상태에서 mutex_를 요구
case WS_EVT_DISCONNECT:
    handleDisconnect(client->id());
    break;

// ✅ 고친 뒤 — id만 적어두고 즉시 빠져나온다
case WS_EVT_DISCONNECT:
...
void GameServer::loop() {
    // 라이브러리 락을 안 잡은 "안전한 시점"에 밀린 일을 처리한다
...
대기열 자체는 작은 배열이라 무거운 뮤텍스 대신 **스핀락(`portMUX_TYPE`)** 으로 보호합니다.
...
⚠️ **스핀락 구간에서는 절대 다른 락을 잡거나 오래 걸리는 일을 하면 안 됩니다.** 인터럽트가 잠시 꺼지기 때문입니다. 배열에 숫자 하나 넣는 정도만 하세요.

💡 **처리가 늦어지지 않나?** `loop()`는 초당 수만 번 돌기 때문에 지연은 마이크로초 단위입니다. 그 사이에 `broadcastState`가 이미 죽은 클라이언트에게 보내려 해도 `ws_.client()`가 `nullptr`을 반환하므로 안전하게 무시됩니다.

#### 📌 여기서 반드시 가져갈 교훈

1. **내 락만 보면 안 됩니다.** 쓰는 라이브러리도 락을 갖고 있고, 그것과 엮이면 데드락이 납니다.
2. **콜백 안에서 락을 잡을 때는 "이 콜백을 누가, 어떤 상태에서 부르는가"를 확인하세요.** 문서에 안 적혀 있으면 **소스를 여세요.** 오픈소스를 쓰는 큰 이유 중 하나가 이것입니다.
3. **데드락을 없애는 유일한 확실한 방법은 "락 획득 순서를 전역적으로 하나로 정하는 것"입니다.** 순서를 지킬 수 없는 코드가 있다면, 그 코드가 락을 안 잡도록(= 나중에 처리하도록) 구조를 바꾸세요.
4. **"나중에 처리하기(deferred work)"는 임베디드에서 매우 강력한 패턴입니다.** 인터럽트 핸들러에서도 똑같이 씁니다 — 짧게 기록만 하고, 무거운 일은 안전한 문맥에서.
...
### 이번 검토에서 고친 버그의 회귀 테스트

한 번 고친 버그는 **다시 생기기 쉽습니다.** 아래는 부록 B의 수정 사항을 확인하는 시나리오입니다. 코드를 손댈 때마다 이 다섯 개를 다시 돌려보세요.

| # | 시나리오 | 기대 결과 | 관련 |
|---|---|---|---|
| R1 | 3명 참가 → 게임 시작 → **1명이 폰 Wi-Fi 끔** → 게임 시작 다시 누름 | 술래 표시가 **정확히 1명** | B-1 |
| R2 | 컨트롤러로 채널 A 참가 → 같은 폰에서 채널 B로 참가 | A 채널의 내 캐릭터가 **회색(끊김)** 으로 바뀌고 30초 뒤 사라짐 | B-2 |
| R3 | 술래와 도망자가 **정면으로 마주 보고** 서로에게 이동 | 통과하지 않고 **잡힘** | B-3 |
| R4 | 4명 이상으로 게임 시작을 여러 번 반복 | 시작 위치가 매번 **맵 곳곳에 흩어짐** (한 줄로 안 뭉침) | B-4 |
| R5 | 게임 중 폰 Wi-Fi를 끄고 20초 대기 후 다시 켬 | 장애물 안에 갇히지 않고 **원래 자리 근처로 복귀** | B-6 |
| R6 | **게임 진행 중** 여러 폰을 반복적으로 껐다 켜기 (10분 이상) | 보드가 멈추지 않고 계속 동작. 시리얼 로그가 끊기지 않음 | B-10-1 (데드락) |
| R7 | 8명 정원을 채운 뒤 9번째 참가 시도 → 한 명 나가고 다시 시도 | 9번째가 **정상 참가됨** (슬롯이 새지 않음) | B-10-2 |

⚠️ **R6은 시간이 걸리는 테스트입니다.** 데드락은 타이밍이 맞아야 터지므로, 짧게 해보고 "괜찮네" 하면 안 됩니다. 접속/해제를 계속 반복하면서 **오래 돌려야** 의미가 있습니다. 멈추면 시리얼 출력이 완전히 끊기는 것으로 확인됩니다.

💡 **회귀 테스트(regression test)** 란 "예전에 고친 버그가 되살아났는지 확인하는 테스트"입니다. 자동화된 테스트를 짜기 어려운 임베디드/게임 프로젝트에서는, 이렇게 **손으로 밟을 시나리오 목록을 문서로 관리하는 것만으로도 큰 효과**가 있습니다.
...
| **"연결됨"인데 화면이 멈춤** | 좀비 연결 (Wi-Fi 범위 이탈 / 보드 전원 차단) | 13단계 하트비트가 25초 안에 감지해 자동 재연결. 그래도 안 되면 새로고침 |
| 술래 표시가 2명 | 이전 판의 `isTagger`가 안 지워짐 | 부록 B-1에서 수정됨. `handleStartGame`이 `active` 전체를 도는지 확인 |
| 자리가 8개인데 더 못 들어감 | 유령 플레이어가 슬롯 점유 | 부록 B-2에서 수정됨. 시리얼에서 `연결 끊김` 로그가 제대로 찍히는지 확인 |
| `git diff`에 잡파일이 수백 줄 | 자동 생성 파일이 추적됨 | 3.5단계 "git에 넣으면 안 되는 것들" 참고 |
...
**동작하는 코드에도 빈틈은 있습니다.** 이걸 숨기지 않고 목록으로 관리하는 것이 좋은 개발 습관입니다.

이 목록은 **실제 코드 전체 검토에서 나온 것**이며, 아래 ✅ 표시된 항목들은 검토 후 **실제로 고쳤습니다.** 고치기 전/후를 비교해보면 "왜 그게 버그였는지"가 훨씬 잘 보입니다.

### ✅ 검토에서 발견해 고친 것들

**B-1. 술래가 2명이 되는 버그** 🔥 *가장 심각했던 문제*
`handleStartGame()`이 위치와 `isTagger`를 **접속 중인 플레이어에 대해서만** 초기화했습니다.

```cpp
// ❌ 고치기 전 — connectedIndexes(접속자)만 순회
...
잠깐 연결이 끊긴 플레이어(`active && !connected`)는 이 루프에서 빠지므로, **이전 판의 `isTagger = true`를 그대로 들고 있습니다.** 새 술래가 뽑히면 화면에 술래 표시가 2명이 되고, 그 사람이 재접속하면 자기가 술래인 줄 압니다.

→ **`connected`가 아니라 `active` 전체를 순회하도록 수정.** 6단계에서 배운 `active`/`connected` 구분을 **정작 여기서 잘못 쓴 것**이 원인입니다. 두 개념을 나눠놨다면, 매번 "여기서 필요한 건 둘 중 어느 쪽인가"를 의식적으로 골라야 합니다.

**B-2. 다시 join하면 이전 방에 "유령 플레이어"가 남음** 🔥
같은 WebSocket이 `join`을 다시 보내면(다른 채널로 갈아타기 등) `freeConnection()`으로 연결 정보만 지웠습니다. 그러면 이전 방의 `Player`는 `connected = true`인 채로 남습니다.

- 조작할 사람이 없으니 영원히 안 움직이고
- `cleanupRooms()`는 `!connected`인 플레이어만 지우므로 **영원히 제거되지 않고**
- 8개뿐인 슬롯을 계속 차지합니다

→ 연결 정보를 지우기 **전에** 그 플레이어를 "끊긴 것"으로 먼저 처리하도록 수정.

**B-3. 술래가 마주 오는 도망자를 관통해 지나감** 🔥
9단계에서 "연습 문제"로 남겼던 것이 사실은 **게임 규칙을 깨는 버그**였습니다.

```
[틱 시작]  술래(5,5)→오른쪽     도망자(6,5)→왼쪽
[목적지]   술래→(6,5)          도망자→(5,5)
[적용 후]  술래(6,5)           도망자(5,5)     ← 서로 자리를 맞바꿈
[잡기판정] 좌표가 다르므로 → 안 잡힘 ❌
```

3단계의 칸 예약은 "같은 칸을 노리는 경우"만 막지, **서로가 떠날 칸을 노리는 경우**는 막지 못합니다. 정면으로 달려오는 술래를 통과해서 지나갈 수 있으니 게임이 성립하지 않습니다.

→ **2.5단계 "자리 바꾸기 검사"를 새로 추가.** 규칙은 이렇게 정했습니다.
- 술래가 낀 경우 → **도망자만 제자리에 세운다.** 술래가 그 칸으로 들어와 정상적으로 잡힘 (정면으로 뛰어들면 잡히는 게 직관에 맞음)
- 도망자끼리면 → 둘 다 제자리 (서로 부딪혀 튕김)

**B-4. 시작 위치가 한 줄로 뭉침**
`findFreeStartPosition()`이 좌상단부터 훑어 첫 빈 칸을 줘서, 게임을 시작하면 전원이 `(1,1), (2,1), (3,1)...`로 늘어섰습니다. 술래 바로 옆에서 시작하는 사람이 생겨 불공평했습니다.

→ **저수지 표본 추출(reservoir sampling)** 로 무작위 배치. 추가 배열 없이 한 번만 훑으면서 균등하게 뽑는 기법입니다.

```cpp
seen++;
if (esp_random() % seen == 0) { outX = x; outY = y; found = true; }
```
n번째 후보를 `1/n` 확률로 채택하면, 끝까지 훑었을 때 **모든 칸이 정확히 같은 확률**이 됩니다. 빈 칸이 몇 개인지 미리 몰라도 되고 메모리도 안 씁니다. 외워둘 가치가 있는 기법입니다.

**B-5. 자리를 못 찾으면 `(1,1)`에 강제 배치**
```cpp
p.x = found ? sx : 1;   // ❌ 못 찾으면 무조건 (1,1)
```
그 자리가 장애물이면 **"플레이어는 장애물 위에 있을 수 없다"는 코드 전체의 전제(불변식)가 깨집니다.**
→ 자리를 못 찾으면 참가를 거절하도록 수정. **잘못된 상태로 계속 진행하느니 실패하는 게 낫습니다.**

**B-6. 끊긴 플레이어 위에 장애물이 생김**
`isPositionOccupied()`가 `connected`인 사람만 셌습니다. 잠깐 끊긴 플레이어의 칸은 "비었다"고 판단되어 장애물이 놓이고, 재접속하면 **장애물 안에 갇힌 채로** 시작합니다.
→ `connected` 조건을 빼고 `active` 전체를 세도록 수정.

**B-7. 재시작해도 맵이 초기화되지 않음**
`initRoomMap()`이 방 생성 시에만 호출되어, 여러 판을 할수록 장애물이 변형된 채로 누적됐습니다.
→ `handleStartGame()`에서 맵을 원래대로 되돌리고, `lastObstacleUpdate`도 같이 초기화(시작하자마자 장애물이 바뀌는 것 방지).

**B-8. 죽은 코드 제거**
장애물 **제거** 경로의 `isPositionOccupied()` 검사는 플레이어가 애초에 장애물 칸에 있을 수 없으므로 **항상 참**이었습니다(주석은 "인접해있으면"이라고 잘못 적혀 있었음).
→ 삭제하고, 불변식을 실제로 지켜주는 **배치 쪽 검사**가 진짜라는 점을 주석으로 명시.

> 💡 **죽은 코드는 없는 코드보다 나쁩니다.** 읽는 사람이 "이 검사가 뭔가를 막고 있구나"라고 믿게 만들기 때문입니다.

**B-9. 안 쓰이던 `ping`/`pong`을 실제로 활용**
Wi-Fi 범위를 벗어나거나 보드가 갑자기 꺼지면 TCP 종료 신호가 오지 않아, 브라우저는 **연결이 살아있다고 착각한 채 아무 것도 못 받는 "좀비 연결"** 이 됩니다. `onclose`가 안 불리니 자동 재연결도 동작하지 않아 화면이 그냥 멈춥니다.
→ `common.js`에 하트비트 추가: 10초마다 `ping`을 보내고, **25초간 아무 메시지도 못 받으면 직접 `close()`** 해서 재연결 경로를 태웁니다.

💡 **핵심 요령**: `pong`만 세지 말고 **어떤 메시지든 받으면 "살아있음"으로 갱신**하세요. 게임 중에는 `state`가 계속 오므로 굳이 ping을 기다릴 필요가 없습니다.

**B-10. 알림을 오류처럼 표시**
"철수 님이 잡혔습니다"를 `showError()`로 띄워서 **붉은 오류 색**으로 보였고, 문구도 `"철수 님이 잡혔습니다! 새 술래: 철수"`처럼 같은 이름이 두 번 나왔습니다.
→ `showNotice()`를 분리하고, `previousTaggerPlayerId`로 잡은 사람을 찾아 `"영희 님이 철수 님을 잡았습니다!"`로 수정.

**B-10-1. 🔥 락 순서 역전으로 인한 데드락** — *서버 검토에서 나온 가장 심각한 문제*
`ESPAsyncWebServer`가 **자기 락을 잡은 채로** `WS_EVT_DISCONNECT` 콜백을 부르는데, 우리 핸들러가 거기서 `mutex_`를 잡았습니다. 반대로 게임 틱은 `mutex_`를 잡고 라이브러리 락을 요구합니다. 두 태스크가 서로를 영원히 기다려 **보드가 통째로 멈춥니다.**
→ `WS_EVT_DISCONNECT`에서는 clientId만 대기열에 적고, 실제 처리는 `loop()`에서 하도록 변경. 자세한 내용은 **15단계**에 있습니다.

**B-10-2. 연결 슬롯 확보 실패 시 유령 플레이어** (B-2와 같은 종류)
`handleJoin()`이 플레이어를 다 만든 **뒤에** `allocConnection()`을 호출했습니다. 그게 실패하면 `connected=true`인데 연결 정보가 없는 플레이어가 남아, `handleDisconnect()`도 `cleanupRooms()`도 손대지 못하는 영구 유령이 됩니다.
→ **연결 슬롯을 상태 변경 "전에" 확보**하도록 순서 변경 + 이후 실패 경로에서 반납.

💡 **일반 원칙 — 자원은 상태를 바꾸기 전에 확보하세요.** "만들고 나서 확보 실패하면 되돌리기"보다 "먼저 확보하고, 안 되면 아무것도 안 건드리기"가 훨씬 안전합니다. 되돌리는 코드는 반드시 빠뜨리는 경로가 생깁니다.

### 🔴 아직 남아있는 것 — 직접 고쳐볼 과제

**B-11. 플레이어가 완전히 갇힐 수 있음**
후진이 없으므로 사방이 막히면 회전만 하다 영영 못 움직입니다.
→ 장애물 배치 전에 "이 칸을 막아도 이동 가능한 칸이 남는가"를 BFS로 검사, 또는 갇힌 플레이어를 빈 칸으로 순간이동.

**B-12. 끊긴 플레이어가 이동 판정에서는 "유령"**
`!connected`인 플레이어는 칸 예약과 잡기 판정에서 빠져, 살아있는 사람이 그 위를 지나갑니다. **화면에는 회색으로 보이는데 통과되는** 상태입니다.
(장애물 배치와 시작 위치 배정에서는 B-6으로 고쳐졌지만, 이동 판정은 일부러 그대로 뒀습니다 — 조작 못 하는 사람이 길을 막으면 게임이 답답해지기 때문입니다.)
→ 어느 쪽이 옳은지는 **게임 디자인 결정**입니다. 정한 뒤 반드시 문서에 남기세요.

**B-13. 게임 중에도 `startGame`이 먹힘**
진행 중에 "게임 시작"을 누르면 맵과 위치가 리셋되고 술래가 다시 뽑힙니다. 지금은 **"다시 시작" 기능으로 정상 동작**하도록 고쳤지만, 실수로 누르는 것을 막고 싶다면 버튼 이름을 바꾸거나 확인 창을 넣으세요.

**B-14. 락을 잡은 채로 네트워크 전송**
`broadcastState()`가 여전히 뮤텍스 안에서 `client->text()`를 호출합니다. **데드락은 B-10-1로 해소됐지만**(락 순서가 하나로 통일됨), 전송이 느려지면 그동안 메시지 처리가 밀리는 문제는 남아 있습니다.
→ 락 안에서는 JSON 문자열만 만들어 "보낼 목록"에 쌓아두고, 락을 푼 뒤 실제 전송하도록 분리(outbox 패턴).

**B-15. `Take`/`Give` 짝 맞추기가 수동**
...
**B-17. 화면이 항상 초당 12.5회 다시 그려짐**
아무도 안 움직여도 `setInterval(draw, 80)`이 계속 돕니다.
→ `requestAnimationFrame`으로 바꾸면 탭이 가려졌을 때 브라우저가 자동으로 멈춰줍니다.

**B-18. `favicon.ico` 404**
브라우저가 페이지마다 자동으로 요청하는데 파일이 없어서, 접속할 때마다 시리얼에 `[WARN] 404 Not Found: /favicon.ico`가 찍혀 로그가 지저분해집니다.
→ `data/favicon.ico`를 넣거나, 각 HTML의 `<head>`에 `<link rel="icon" href="data:,">`를 추가.
